    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                page = 1
                while True:
                    result = RecipeService.search_recipes_by_budget(
                        budget, quarter, page
                    )
                ...
                allergy = get_user_input(
                    "알레르기 정보를 입력하세요 (엔터: 내 알레르기 정보 사용, 쉼표로 구분): "
                )
                ...
                page = 1
                while True:
                    result = RecipeService.search_recipes_by_allergy(
                        allergy, quarter, page
                    )
                    show_more = display_recipes(result)
                    if not show_more:
                        break
            ...
            if choice == "1":
                ...
                if quarter:
                    if not (1 <= int(quarter) <= 4):
                        print("올바른 분기를 입력하세요 (1-4)")
                        continue
                    results = PriceService.get_ingredient_price_by_quarter(
                        ingredient_name, int(quarter)
                    )
                else:
                    results = PriceService.get_ingredient_price_by_quarter(
                        ingredient_name
                ...
                if quarter:
                    if not (1 <= int(quarter) <= 4):
                        print("올바른 분기를 입력하세요 (1-4)")
                        continue
                    results = PriceService.get_recipe_price_by_quarter(
                        recipe_name, int(quarter)
                    )
                else:
                ...
                if subchoice == "1":
                    ...
                    results = PriceService.analyze_price_trend(
...
def main():
    ...
    # 뷰 생성
    RecipeService.create_budget_view()
